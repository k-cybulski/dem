...
def compare_states_jax_torch(state_jax, state_torch):
    ...
    for attr in [
        ...
        if not np.isclose(
            getattr(state_jax, attr), getattr(state_torch, attr).detach().numpy()
        ).all():
            print(f"{attr} differ")
    ...
    for attr in [
        ...
        if not np.isclose(
            getattr(state_jax.input, attr),
            getattr(state_torch.input, attr).detach().numpy(),
        ).all():
...
def test_equivalence_jax_batched():
    ...
    np.concatenate([A.reshape(-1), B.reshape(-1), C.reshape(-1)])
    ...
    def g_jax(x, v, params):
        C = params[
            (m_x * m_x + m_x * m_v) : (m_x * m_x + m_x * m_v + m_y * m_x)
        ].reshape((m_y, m_x))
        return jnp.matmul(C, x)

    jit(
        lambda mu_x_tildes, mu_v_tildes, params: generalized_func(
            f_jax, mu_x_tildes, mu_v_tildes, m_x, m_v, p, params
        )
    )
    jit(
        lambda mu_x_tildes, mu_v_tildes, params: generalized_func(
            g_jax, mu_x_tildes, mu_v_tildes, m_x, m_v, p, params
        )
    )

    x_tildes = jnp.stack(list(iterate_generalized(xs, dt, p, p_comp=p_comp))).astype(
        jnp.float64
    )
    v_tildes = jnp.stack(list(iterate_generalized(vs, dt, d, p_comp=p_comp))).astype(
        jnp.float64
    )
    ...
    eta_theta = np.concatenate(
        [rng.uniform(-2, 2, m_x * m_x + m_x * m_v), C.reshape(-1)]
    )
    ...
    v_autocorr = noise_cov_gen_theoretical(
        d, sig=noise_temporal_sig, autocorr=autocorr_friston()
    )
    v_autocorr_inv = np.linalg.inv(v_autocorr)

    noise_autocorr = noise_cov_gen_theoretical(
        p, sig=noise_temporal_sig, autocorr=autocorr_friston()
    )
    ...
    np.stack(list(iterate_generalized(ys, dt, p, p_comp=p_comp)))
    ...
    mu_x_tildes = x_tildes
    mu_v_tildes = v_tildes
    x_tildes[0]  # groundtruth

    np.tile(np.kron(noise_autocorr_inv, np.eye(m_x)), (len(mu_v_tildes), 1, 1))
    vmap(jnp.linalg.inv)(p_v_tildes)

    jnp.linalg.inv(p_theta)
    jnp.linalg.inv(p_lambda)

    mu_v_tildes[0]

    dem_input = DEMInputJAX(
        dt=dt,
        m_x=m_x,
        m_v=m_v,
        m_y=m_y,
    ...
    batched_input_dict = {
        ...
        "g": g_torch,
        **{
            attr: dem_input.__dict__[attr]
            for attr in ["dt", "m_x", "m_v", "m_y", "p", "d", "d_comp"]
        },
        **{
            attr: torch.from_numpy(np.array(dem_input.__dict__[attr])).to(
    ...
    dem_state_b = DEMStateBatched.from_input(
        dem_input_b,
        torch.from_numpy(np.array(x0)).to(dtype=TORCH_DTYPE, device=TORCH_DEVICE),
    )

    dem_state_b.mu_x_tildes = torch.from_numpy(np.array(mu_x_tildes)).to(
        dtype=TORCH_DTYPE, device=TORCH_DEVICE
    )
    dem_state_b.mu_v_tildes = torch.from_numpy(np.array(mu_v_tildes)).to(
        dtype=TORCH_DTYPE, device=TORCH_DEVICE
    )
    dem_state_b.mu_x0_tilde = torch.from_numpy(np.array(mu_x_tildes[0])).to(
        dtype=TORCH_DTYPE, device=TORCH_DEVICE
    )
    dem_state_b.mu_v0_tilde = torch.from_numpy(np.array(mu_v_tildes[0])).to(
        dtype=TORCH_DTYPE, device=TORCH_DEVICE
    )
