...
def autocorr_friston():
    # Autocorrelation function of a Gaussian process generated by convolving
    ...
    for i in range(n):
        for j in range(n):
            if (i + j) % 2 == 1:
                ...
                sign = -2 * ((i % 2) == 1) + 1
                matr[i, j] = sign * diffed.evalf(
                    subs={sym.Symbol("x"): 0, sym.Symbol("sigma"): sig}
                )
    return matr
