...
def generalized_func(func, mu_x_tildes, mu_v_tildes, m_x, m_v, p, params):
    ...
    mu_x_grad = torch.autograd.functional.jacobian(
        lambda x: func(x, mu_v, params), mu_x, create_graph=True
    )
    mu_v_grad = torch.autograd.functional.jacobian(
        lambda v: func(mu_x, v, params), mu_v, create_graph=True
    )
    ...
    func_appl_d_x = torch.einsum(
        "bkj,bdj->bdk", mu_x_grad, mu_x_tildes_r[:, 1:, :]
    ).reshape((n_batch, -1, 1))
    func_appl_d_v = torch.einsum(
...
def internal_energy_dynamic(
    ...
    def _int_eng_dynamic(
        ...
        g_tildes = generalized_func(
            g, mu_x_tildes, mu_v_tildes_pad, m_x, m_v, p, mu_theta
        )
        f_tildes = generalized_func(
            f, mu_x_tildes, mu_v_tildes_pad, m_x, m_v, p, mu_theta
        )
        ...
        u_t_y_ = -torch.bmm(err_y.mT, torch.matmul(prec_z_tilde, err_y)).reshape(
            n_batch
        ) + torch.logdet(prec_z_tilde)
        u_t_v_ = -torch.bmm(err_v.mT, torch.bmm(p_v_tildes, err_v)).reshape(
            n_batch
        ) + torch.logdet(p_v_tildes)
        u_t_x_ = -torch.bmm(err_x.mT, torch.matmul(prec_w_tilde, err_x)).reshape(
            n_batch
        ) + torch.logdet(prec_w_tilde)
    ...
    u_t_x_tilde_dd = torch.autograd.functional.hessian(
        lambda mu: torch.sum(_int_eng_dynamic(mu, mu_v_tildes, mu_theta, mu_lambda)),
        mu_x_tildes,
        create_graph=True,
    )
    u_t_v_tilde_dd = torch.autograd.functional.hessian(
        lambda mu: torch.sum(_int_eng_dynamic(mu_x_tildes, mu, mu_theta, mu_lambda)),
        mu_v_tildes,
        create_graph=True,
    )
    u_t_theta_dd = torch.autograd.functional.hessian(
        lambda mu: torch.sum(_int_eng_dynamic(mu_x_tildes, mu_v_tildes, mu, mu_lambda)),
        mu_theta,
        create_graph=True,
    )
    u_t_lambda_dd = torch.autograd.functional.hessian(
        lambda mu: torch.sum(_int_eng_dynamic(mu_x_tildes, mu_v_tildes, mu_theta, mu)),
        mu_lambda,
        create_graph=True,
    )
...
def internal_action(
    ...
    u, u_theta_dd, u_lambda_dd = internal_energy_static(
        mu_theta, mu_lambda, eta_theta, eta_lambda, p_theta, p_lambda, compute_dds=True
    )
    (
        u_t,
        u_t_x_tilde_dds,
...
def free_action(
    ...
    # dynamic terms
    mu_x_tildes,
    mu_v_tildes,  # iterator of state and input mean estimates in generalized coordinates
    sig_x_tildes,
    sig_v_tildes,  # as above but for covariance estimates
    y_tildes,  # iterator of outputs in generalized coordinates
    eta_v_tildes,
    p_v_tildes,  # iterator of input mean priors
    ...
    sig_logdet_t = (
        torch.sum(torch.logdet(sig_x_tildes)) + torch.sum(torch.logdet(sig_v_tildes))
    ) / 2
    f_tsum = (
        torch.sum(u_t)
        + sig_logdet_t
...
class DEMStateBatched:
    ...
    def from_input(
        ...
        input: DEMInputBatched,
        x0: torch.Tensor,
        mu_theta: torch.Tensor = None,
        **kwargs,
    ):
        x0 = x0.reshape(-1)
        assert len(x0) == input.m_x

        mu_x0_tilde = torch.concat(
            [
                x0,
                torch.zeros(
                    input.p * input.m_x, dtype=input.dtype, device=input.device
                ),
            ]
        ).reshape((-1, 1))
        ...
        sig_x_tildes = kron(
            input.noise_autocorr_inv,
            torch.eye(input.m_x, dtype=input.dtype, device=input.device),
        ).repeat(len(mu_v_tildes), 1, 1)
    ...
    def step_d(state, lr):
        ...
        deriv_mat_x = torch.from_numpy(deriv_mat(state.input.p, state.input.m_x)).to(
            dtype=state.input.dtype, device=state.input.device
        )
        deriv_mat_v = torch.from_numpy(deriv_mat(state.input.d, state.input.m_v)).to(
            dtype=state.input.dtype, device=state.input.device
        )
        for t, (y_tilde, sig_x_tilde, sig_v_tilde, eta_v_tilde, p_v_tilde) in tqdm(
            enumerate(
                zip(
            ...
            total=len(state.input.p_v_tildes),
            desc="Step D",
        ):
            ...
            x_d = (
                deriv_mat_x @ mu_x_tilde_t
                + lr * torch.autograd.grad(f_eng, mu_x_tilde_t, retain_graph=True)[0]
            )
            v_d = (
                deriv_mat_v @ mu_v_tilde_t
                + lr * torch.autograd.grad(f_eng, mu_v_tilde_t)[0]
            )
            x_dd = lr * torch.autograd.functional.hessian(
                lambda mu: dynamic_free_energy(mu, mu_v_tilde_t), mu_x_tilde_t
            )
            v_dd = lr * torch.autograd.functional.hessian(
                lambda mu: dynamic_free_energy(mu_x_tilde_t, mu), mu_v_tilde_t
            )
            x_dd = deriv_mat_x + _fix_grad_shape(x_dd)
            v_dd = deriv_mat_v + _fix_grad_shape(v_dd)
            step_matrix_x = (
                torch.matrix_exp(x_dd * state.input.dt)
                - torch.eye(
                    x_dd.shape[0], dtype=state.input.dtype, device=state.input.device
                )
            ) @ torch.linalg.inv(x_dd)
            step_matrix_v = (
                torch.matrix_exp(v_dd * state.input.dt)
                - torch.eye(
                    v_dd.shape[0], dtype=state.input.dtype, device=state.input.device
                )
            ) @ torch.linalg.inv(v_dd)
    ...
    def step_precision(state):
        ...
        (
            ...
            u_t_x_tilde_dds,
            u_t_v_tilde_dds,
        ) = state.internal_action()
        state.sig_theta = torch.linalg.inv(-u_theta_dd)
        state.sig_lambda = torch.linalg.inv(-u_lambda_dd)
        state.sig_x_tildes = torch.stack(
            [-torch.linalg.inv(u_t_x_tilde_dd) for u_t_x_tilde_dd in u_t_x_tilde_dds]
        )
        state.sig_v_tildes = torch.stack(
            [-torch.linalg.inv(u_t_v_tilde_dd) for u_t_v_tilde_dd in u_t_v_tilde_dds]
        )
    ...
    def step_m(state, lr_lambda, iter_lambda, min_improv):
        ...
        for i in tqdm(range(iter_lambda), desc="Step M"):
            ...
            lambda_dd = lr_lambda * torch.autograd.functional.hessian(
                lambda_free_action, state.mu_lambda
            )
            step_matrix = (
                torch.matrix_exp(lambda_dd)
                - torch.eye(
                    lambda_dd.shape[0],
                    dtype=state.input.dtype,
                    device=state.input.device,
                )
            ) @ torch.linalg.inv(lambda_dd)
    ...
    def step_e(state, lr_theta):
        ...
        theta_dd = lr_theta * torch.autograd.functional.hessian(
            theta_free_action, state.mu_theta
        )
        step_matrix = (
            torch.matrix_exp(theta_dd)
            - torch.eye(
                theta_dd.shape[0], dtype=state.input.dtype, device=state.input.device
            )
        ) @ torch.linalg.inv(theta_dd)
