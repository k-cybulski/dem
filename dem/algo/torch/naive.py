...
from .util import _fix_grad_shape, clear_gradients_on_state, kron
...
def generalized_func(func, mu_x_tilde, mu_v_tilde, m_x, m_v, p, params):
    ...
    mu_x_grad, mu_v_grad = torch.autograd.functional.jacobian(
        lambda x, v: func(x, v, params), (mu_x, mu_v), create_graph=True
    )
...
def internal_energy_static(
    ...
    if compute_dds:
        u_c_theta_dd = torch.autograd.functional.hessian(
            lambda mu: _int_eng_par_static(mu, eta_theta, p_theta),
            mu_theta,
            create_graph=True,
        )
        u_c_lambda_dd = torch.autograd.functional.hessian(
...
def internal_energy_dynamic(
    ...
    if compute_dds:
        u_t_x_tilde_dd = torch.autograd.functional.hessian(
            lambda mu: _int_eng_dynamic(mu, mu_v_tilde, mu_theta, mu_lambda),
            mu_x_tilde,
            create_graph=True,
        )
        u_t_v_tilde_dd = torch.autograd.functional.hessian(
            lambda mu: _int_eng_dynamic(mu_x_tilde, mu, mu_theta, mu_lambda),
            mu_v_tilde,
            create_graph=True,
        )
        u_t_theta_dd = torch.autograd.functional.hessian(
            lambda mu: _int_eng_dynamic(mu_x_tilde, mu_v_tilde, mu, mu_lambda),
            mu_theta,
            create_graph=True,
        )
        u_t_lambda_dd = torch.autograd.functional.hessian(
            lambda mu: _int_eng_dynamic(mu_x_tilde, mu_v_tilde, mu_theta, mu),
            mu_lambda,
            create_graph=True,
        )
...
class DEMStateNaive:
    ...
    def step_d(state, lr, benchmark=False):
        ...
        if benchmark:
            benchmark_t0 = time()
            benchmark_ts = []
        for t, (y_tilde, sig_x_tilde, sig_v_tilde, eta_v_tilde, p_v_tilde) in enumerate(
            zip(
            ...
            step_matrix_x = (
                torch.matrix_exp(x_dd) - torch.eye(x_dd.shape[0])
            ) @ torch.linalg.inv(x_dd)
            step_matrix_v = (
                torch.matrix_exp(v_dd) - torch.eye(v_dd.shape[0])
    ...
    def step_m(state, lr_lambda, iter_lambda, min_improv, benchmark=False):
        ...
        for i in range(iter_lambda):
            ...
            step_matrix = (
                torch.matrix_exp(lambda_dd) - torch.eye(lambda_dd.shape[0])
    ...
    def step_e(state, lr_theta):
        ...
        step_matrix = (
            torch.matrix_exp(theta_dd) - torch.eye(theta_dd.shape[0])
    ...
    def step(
        ...
        benchmark_ts_m = state.step_m(
            lr_lambda, iter_lambda, min_improv=m_min_improv, benchmark=benchmark
        )
        if benchmark:
            benchmark_t0 = time()
